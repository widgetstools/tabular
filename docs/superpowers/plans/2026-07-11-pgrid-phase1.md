...
| 2026-07-12 | Post-T10: sticky revert + always-visible scrollbars (user-reported, 4th round) | ✅ Real-input verification: 12px classic gutters at rest (both axes), track-click paged 0→433px, 85/85 wheel frame pairs with row motion exactly matching scroll deltas (native), 100% coverage at human scroll speeds; commit `9612805` | User: the pre-sticky build was "100% just like finos"; sticky broke track-click paging and made trackpad scroll feel row-by-row/page-by-page. The sticky theory over-corrected: making ALL motion main-thread-owned (regular-table's model) trades away native compositor smoothness, and users feel that trade immediately. Reverted to the validated architecture — layer in scroll space (native motion), atomic paints, stale ride + edge-pin — keeping row overscan 12. macOS scrollbar visibility: styling `::-webkit-scrollbar` (regular-table's material recipe: transparent track, content-box thumb, 12px) replaces overlay scrollbars with classic always-visible ones AND restores track-click paging. **Lesson: when the user says a prior build was right, revert first and re-derive — don't defend the theory.** The compositor-displacement risk the sticky change targeted is real but subordinate: with per-rAF glue writes it amounts to at most one frame of displacement, imperceptible next to losing native motion. Follow-up (`ceddab5`): the same always-visible-scrollbar treatment applied to the showcase nav sidebar and pgrid's columns sidebar (user couldn't reach lower nav items — macOS overlay bars gave no affordance); note `scrollbar-width: thin` disables `::-webkit-scrollbar` styling in modern Chrome and had to be removed. |
| 2026-07-12 | Post-T10: compositor-proof scrolling (user-reported, 3rd round) | ✅ Real mouse.wheel verification (isolated tab, full feed): vertical 763 frames, 0 backward movements, 100% min/avg coverage through steady+aggressive wheel; horizontal same coverage; commit `7169cf1` + research-doc addendum | User reported flicker/jerk/blank persisting after the scroll fixes, then asked for vertical/horizontal parity. Root cause the synthetic tests could never see: real scrolling is ASYNC compositor scrolling — it paints scroll-space content displaced before the main-thread sync runs, once per input. Structural fix (regular-table's actual load-bearing property): the row layer is now `position: sticky` (compositor-pinned; the clip-pin equivalent), rows viewport-relative, both axes' motion written solely by sync (`placeLayer(y, scrollLeft)`); glue/edge-pin logic carried over in content-space form. Row overscan 12 vs column 4 (windows move 26px vs ~120px per step) for axis-parity of the fetch-free band. Lessons: (1) never put virtualized content in scroll space under async scrolling; (2) verify scroll behavior ONLY with real input (`mouse.wheel` via playwright run-code), never dispatchEvent/scrollTop writes; (3) the harness's ×desk click can land while the engine still digests the snapshot — wait for spacer ≥ grouped height, then > 100k after the transition. |
| 2026-07-12 | Post-T10: rubberband fix + regular-table deep dive (user-reported/requested) | ✅ 0 backward row movements across 485 frames of sustained wheel scroll (was 144, max 80px); flick coverage 100/100 with settle 100%; deep-dive doc `docs/research/2026-07-12-regular-table-scroll-study.md`; commits `d8fb249` + docs | User reported rubberbanding ("scrolls a little up then down"). Root cause: the freeze branch offset stale content from the PREVIOUS sync's scrollTop, so at coverage-escape the layer snapped back by the just-scrolled delta. Fix: hybrid stale handling — stale rows ride natively at true content coordinates (swap lands on identical coords → zero jump), and on escape the stale window's EDGE pins to the viewport edge via a clamp that is continuous at the boundary (backward motion impossible by construction; prevScroll field removed). Deep dive (sources from regular-table's sourcemap): their table is clip-pinned (never in scroll space; every scroll = serialized redraw via throttle_tag; same-window scroll = two CSS-var writes; percent-of-scrollable row mapping; no overscan; learned column widths). pgrid now matches their two guarantees (never-blank viewport, atomic paints) while keeping native compositor scrolling, which their architecture structurally forgoes. |
| 2026-07-12 | Post-T10: atomic paints (user-reported tearing) | ✅ 100% viewport coverage (worst AND avg) warm across half-viewport steps, 1300px flicks, wheel-speed scrolls, and a full-range 20k-row sweep; commit `146c3c3` | User reported the viewport sometimes half-populated during scroll (top half rows, bottom empty) — never happens in FinOS. Root cause: the stale-pixel glue gated on the ANCHOR row only, so a window partially overlapping the last slice took the normal paint path — the overlap stamped, the non-overlap stayed empty, and out-of-window stale slots sat at wrong offsets against the moved layer. Fix: paints are atomic (regular-table's draw semantics) — swap only when data covers the whole window (`rowMeta(firstRow) && rowMeta(min(lastRow, rowCount-1))`, contiguity guaranteed by single-read slices); otherwise stay fully glued. Also learned: when the engine keeps pace (warm, ~150ms cadence) the glue never engages at all — windows swap atomically frame by frame. Cold coverage right after a view rebuild still dips briefly (draw-in; FinOS equivalent) — measure warm. |
| 2026-07-12 | Post-T10: regular-table scroll study (user-requested) | ✅ Sub-cell scrolling now pixel-perfect with zero engine reads and zero re-stamps within an unchanged window (4px steps move rows exactly 4px); full-sweep visibility 20/20 retained; commit `2a6515b` | User asked how the FinOS grid scrolls so smoothly. Extracted regular-table's sources (sourcemap) and read its pipeline: smoothness = (1) content glued to the viewport by construction (sticky/clip model), (2) **same-window scrolls do nothing but write the fractional row offset into one CSS variable** (`update_sub_cell_offset`; the `internal_draw` else-branch) — no DOM stamping, no engine read, (3) draws serialized via `throttle_tag`. Our gap was (2): Task 6 had baked `subCellPx` into every row transform (deviating from spec §6's "offset on the row layer"), so a 1px wheel scroll re-stamped the whole pool AND refetched the identical window from the saturated engine. Fixed: offset moved to the layer transform (uncompressed same-window scrolling is now pure native compositor scrolling — `layerTop` depends only on `firstRow`), and `requestWindow` fires only when the logical window changes. Also diagnosed: cold-transition measurements (right after a view rebuild) show low visibility because `setSize` recreates slots empty — the glue has no pixels yet; warm measurements are the honest ones (matches regular-table's draw-in on config change). |
| 2026-07-12 | Post-T10: OpenFin bench + scroll-continuity fix (user-reported) | ✅ OpenFin (runtime 43.142.104.1, CDP-driven): sweep p50 8.3 / p95 12.4 / worst 61.7 (1 frame >20ms), agg cadence 13/s, rows-visible avg 15.6/24; Chrome scroll continuity 20/20 rows at every sweep sample (was 1/20); commit `57926c1` + addendum update | User asked whether the bench ran in OpenFin (it hadn't — playwright Chrome only) and reported rows invisible during scrolling. (1) **Scroll blanking was real and environment-independent**: fast scrolls outran the ~300ms saturated-engine window reads; the pool hid data-less slots → blank viewport. Fix: in-flight windows glue the last painted pixels to the viewport (layer tracks scroll delta; pool keeps stale slots until a complete frame) — the FinOS stale-window-mid-fetch behavior. (2) **OpenFin bench** ran via the manifest's remote-debugging port with a node CDP driver; first run hit a stale pre-fix page (OpenFin window predated several fixes — reload before benching) producing bogus setup failures; post-reload everything worked, and cadence was 13/s (better than Chrome's 4–6/s — the Chrome session shared the feed with other page connections). One 61.7ms compositor hitch in OpenFin vs none in Chrome. OpenFin window + CDP driver left available; temp bench scripts not committed. |
| 2026-07-12 | Task 10 — **phase 1 complete** | ✅ Bench done (all spec §8 criteria met), spec §10 results addendum written, `packages/pgrid/README.md` created, full gate green (typecheck + all 5 test scripts), commit `d55e1b9`, pushed to origin/main (35 commits) | Bench highlights: pgrid vertical sweep p50 9.1 / p95 12.5 / worst 14.4ms, 0 frames >20ms (382-row desk→ccy tree, 10k updates/s); flat 372-col horizontal p95 12.8, 0 dropped; aggregate repaint 6.0/s push vs FinOS datagrid ~1.4 full-window redraws/s vs SSRM = its polling interval by construction; tick→paint probe 274–465ms (median ~371ms — engine-queue-bound, the P4 seam target); engine heap flat across 20 grouped↔pivoted rebuilds (used_size 3.448→3.455MB then flat) — no view leak. Deviations/notes: (1) SSRM aggregate cadence recorded by construction — its center-container cells weren't instrumentable in this ag-grid version, and its refresh-interval UI (1s/2s/5s/Off) is the definitional answer anyway. (2) FinOS grouped-view cadence measurement was muddied by static count-aggregate columns in the visible window; the flat-view 1.38 redraws/s number (clean, earlier same-session) is the one cited. (3) Node in-process engine threw a detached-ArrayBuffer error at 20k rows (WASM memory growth mid-update); heap check ran at 5k — browser worker engine unaffected at 20k. (4) `window.__pgrid` bench hook added to the page. Bench ran on the shared, user-contested tab — interference-checked per run. |
| 2026-07-12 | Task 9 + fixes (user-reported ×2) | ✅ Full STOMP demo verified live (20k rows × 372 union-schema cols at ~10k updates/s): grouped aggregates tick 9/9 with zero refresh code; 6 consecutive expand/collapse toggles deterministic under load; pivot ticks; 372-col scroll sweep at 120fps, p95 12.8ms, 0 frames >20ms; commits `cc9158d` (feat), `7f5a181` (fixes) | Wrapper + full page per plan (flatten/buildSchema extracted to `stomp/flattenFi.ts`, shared with PerspectiveStress; pgrid tsconfig gains `jsx`). Three behavior changes from user reports/measurement: (1) **No leaf level in pivot mode** — the injected index group level is skipped when split_by is active; `RowMeta.expandable` added; deepest pivot groups hide the chevron and `toggleExpand` blocks them (ag-grid semantics; user report: leaf drill-down of pivoted aggregates is meaningless). (2) **Expand hardening (spec §10 full mitigation)** — first chevron click under 10k updates/s was dropped (~stale expanded flag → collapse no-op); toggleExpand now re-validates against a fresh engine read (which serializes behind pending updates) and re-resolves by path on index shift; 6/6 toggles landed after. (3) **"Choking" report measured, not confirmed** — head-to-head vs the FinOS datagrid on the identical saturated feed: pgrid aggregate repaint cadence 4–6/s vs FinOS 1.4 full-window redraws/s (≈3–4× fresher), flat-view per-cell change fidelity 1:1 (every real change painted; FinOS overwrites whole windows and can skip intermediate values), frame health pgrid worst 14.8ms/0 dropped vs FinOS 38.3ms/1 dropped. Cadence is engine-queue-bound for both; ViewHost now notifies before the rowCount refresh so the window read shares the queue drain. Caveat: interactive verification was contested — the user was live-driving the same shared browser tab during measurement (see Task 8 interference note). Incident: an Edit wrote literal NUL bytes into grid.ts (a join separator intended as an escaped \\u0000 sequence); tsc accepted them but grep/awk went silent — repaired with escaped form. |
| 2026-07-12 | Fix (pivot, user-reported) + FinOS datagrid re-study | ✅ Pivot matrix verified correct under ticks (each desk's values only in its own ccy columns, blanks elsewhere, 0 label leaks, all rows ticking); node tests + full typecheck green; commits `f080c8f`, `a424af0` | User reported pivot "all messed up" and asked for a FinOS datagrid re-read (sources extracted from the npm sourcemap into scratch). Findings: (1) **Empty pivot intersections showed the group label** — Task 5's blank-cell label fallback predates the auto group column and mislabeled legitimately-null cells; removed, null → blank (FinOS `format_cell` returns null for null too). (2) **Split column-set parity** (probed live): updates that introduce a new split value grow `column_paths` immediately, and window responses include them at once — our per-view path cache and response-key-derived columns let data and headers disagree. Now: `window()` keys values off the cached paths with null-fill (the datagrid's `columns[path] \|\| fill(null)` defense), ViewHost refreshes the cache on every update flush when split_by is active, and the grid rebuilds display columns + header when the path fingerprint changes. Regression block added to `scripts/pgrid-view-host.ts`. Non-adopted divergences, deliberate: FinOS's split-view sort rotation has extra `col desc/col asc` states (spec §5.7 chose the simple rotation — phase 2); FinOS renders one row-header column per group level vs our ag-grid-style single auto group column (approach A); FinOS doesn't await expand redraws (spec §5.3 fixes that race). Perspective 3.8 no longer omits empty columns from split windows (probed), but the null-fill guard is kept — it's what makes cache-keyed reads safe mid-update. Follow-up (`297086a`): the harness's `ccy=(i*7)%4` correlated with `desk=i%6`, leaving each desk with only 2 of 4 currencies — the correct-but-sparse pivot read as a bug (user report #2); decorrelated so every desk×ccy intersection has rows, verified full matrix + column-sum cross-check in browser. Follow-up (`a3c3b44`, user report #3): leaf rows now leave the auto group column blank — group values only (ag-grid semantics); the Task 7 adapter had rendered leaf identity (the injected index value) there. |
...
- [x] **Step 1: Implement wrapper** (React optional peer; the file compiles only when consumed via `pgrid/react`).
- [x] **Step 2: Full showcase page.** Reuse the showcase's existing STOMP wiring (`connectFiPositions` + the stress page's `flatten`/`buildSchema` helpers — import from a small shared module, extract if needed): 20k rows × union schema into `grid.setSchema` + `grid.load` + `grid.update` per batch. Default state: group by desk, values mv/pnl/dailyPnl sum + px/yield avg; panels on. Status bar: rows, columns, updates applied, feed badge.
- [x] **Step 3: Browser verify** (the full success criteria, spec §8): grouped aggregates tick with ZERO refresh/polling code; expand under ticks stable; pivot by currency ticks; 372-col horizontal scroll smooth.
- [x] **Step 4: Typecheck + commit** — `git commit -m "feat(pgrid): React wrapper + STOMP-fed showcase page"`.
...
- [x] **Step 1: Bench** with the session's established methodology (programmatic 3s scroll sweeps sampling rAF deltas + aggregate-tick latency): pgrid page vs `Perspective (FinOS)` viewer page vs `Perspective SSRM` ag-grid page, same feed. Record: scroll p50/p95/worst/dropped both axes at 10k updates/s grouped desk→ccy; tick-to-paint latency (update → changed group cell repaint, via a marked row probe); WASM heap before/after 10 view rebuilds (leak check).
- [x] **Step 2: Results addendum** table + interpretation in the spec; README with quickstart, Vite `optimizeDeps.exclude` note, and the P2+ roadmap pointer.
- [x] **Step 3: Final** — `npm run typecheck && npx tsx scripts/pgrid-engine.ts && npx tsx scripts/pgrid-window-math.ts && npx tsx scripts/pgrid-view-compiler.ts && npx tsx scripts/pgrid-view-host.ts && npx tsx scripts/pgrid-materializer.ts`; commit `docs+README`; push.
