...
/** Flat rows + path for the tree data page (desk → book → position). */
...
export function makeTreeRows(seed = 11): TreeRow[] {
  ...
  for (const desk of DESKS) {
    // The desk row itself is a real record (a folder IS a record — §4.14).
    out.push({
      id: `T${id++}`,
      path: [desk],
      instrument: '',
    ...
    for (let b = 0; b < books; b++) {
      ...
      // Note: no explicit row for the book — the grid synthesizes a filler.
