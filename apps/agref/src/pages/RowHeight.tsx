...
function makePeople(n: number): PersonRow[] {
...
function FullWidthCellRenderer(props: { data: PersonRow }) {
  const d = props.data;
  return (
    <div
      style={{
...
/** AG Grid reference — row-height + auto-height + full-width rows docs examples. */
export function RowHeightPage() {
  ...
  const modeOptions = useMemo<GridOptions<PersonRow>>(() => {
    if (mode === 'headers') {
      return {
  ...
  return (
    <main className="page">
      <div className="page-head">
        <h2>Row height &amp; full-width rows (AG Grid)</h2>
        <p>
          ...
          <b>Full width</b>: <code>isFullWidthRow</code> + <code>fullWidthCellRenderer</code>.{' '}
          <b>Header sizing</b>: <code>wrapHeaderText</code> + <code>autoHeaderHeight</code>.
        </p>
      </div>
